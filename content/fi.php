...
session_name ('lgFileindex_Session:'.get_url());
session_start();

$user_names = array( 1=>"Jan",);
$user_passes = array( 1=>"45a1c9e451fa4256842d371338610732");
$user_privileg = array( 1=>true);
$isAdmin = false;
$autoLogin = false;
...
function get_url(){
	...
	return substr($url,0, -strlen(strrchr($url,'/'))+1);
}
 ...
 * @license  GPL v3
 */
function print_index($directory_base_path = './', $exclude = ".|..|./|../|.DS_Store|fi.php|fi.css|index.php|index.html|index.htm|.svn|.htaccess|.htpasswd|doc.png|dir.png", $level_i = 0){
	...
	$result .= '<a ';
	if( strcmp($directory_base_path, './') != 0)
	$result .= 'href="javascript:toggle(\''. $toggle_id.'_content\')"';
	$result .= ' class="name">';
	if( strcmp($directory_base_path, './') == 0) 
	...
	if($directory_base_path != './' && $GLOBALS['isAdmin']) 
	...
	$result .= '</div>';
	if( $level_i % 2)
    	$result .= '<div id="'.$toggle_id.'_content" class="content even"';
	else
		$result .= '<div id="'.$toggle_id.'_content" class="content odd"';
    ...
    if (!$folder_handle = opendir($directory_base_path)) {
        ...
        while( $filename = readdir($folder_handle) ) {
			if( !in_array($filename, $exclude_array) && is_dir( substr($directory_base_path,2) . $filename) && strcmp($filename, ".")!=0 && strcmp($filename, "..")!=0) {				
				$result .= print_index($directory_base_path.$filename, $exclude, $level_i);	
			}
		}
		...
		while($filename = readdir($folder_handle) ) {
			/*}else*/ if( !in_array($filename, $exclude_array) && is_file( substr($directory_base_path,2) . $filename)) {					
					...
					if($GLOBALS['isAdmin'])
						$result .= '<a href="javascript:dialog_toggle2(\'dialog_delete_file\',\'overlay\',\'delete_file_form\',\'delete_filename\',\''.$path.'\')" class="blank">löschen</a>';
					$result .= '</span><br />';
					$result .= '<span>Erstellungsdatum:</span>';
					$result .= '<span>'.date("d.m.Y - H:i",filectime($directory_base_path . $filename)).'</span>';
					$result .= '<span style="float:right;">';
					if($GLOBALS['isAdmin'])
						$result .= '<a href="javascript:dialog_toggle3(\'dialog_rename_file\',\'overlay\',\'rename_file_form\',\'new_filename\',\''.substr(strrchr($path,'/'),1).'\',\'old_filename\',\''.$path.'\')" class="blank">umbenennen</a>';
					$result .= '</span><br />';
					$result .= '<span>Änderungsdatum:</span>';
					$result .= '<span>'.date("d.m.Y - H:i",filemtime($directory_base_path . $filename)).'</span>';
					$result .= '<span style="float:right;">';
					if($GLOBALS['isAdmin'])
						$result .= '<a href="javascript:dialog_toggle2(\'dialog_move_file\',\'overlay\',\'move_file_form\',\'move_filename\',\''.$path.'\')" class="blank">bewegen</a>';
					$result .= '</span>';
					$result .= '</div></div>';
			}
        }
 ...
 * $password has to be md5 crypted
 * requires $user_names, $user_passes
 * @author	Jan Beneke -- lightgraffiti.de
 *
 * @param    $username 		string    username
 * @param    $password 		string    password (md5 crypted)
...
function is_admin( $username, $password){
	extract($GLOBALS);
	$user_key = array_search( $username, $user_names);
	return $user_privileg[$user_key];
}

/**
 * is_user_md5
 * checks login and sets user privileges
 * $password_md5 and $username_md5 has to be md5 crypted
 * requires $user_names, $user_passes
 * @author	Jan Beneke -- lightgraffiti.de
 *
 * @param    $username 		string    username (md5 crypted)
 * @param    $password 		string    password (md5 crypted)
 * @return   true/false	  	boolean	  true if login is okay
 * @access 	 public
 * @license  GPL v3
...
function is_user_md5( $username_md5, $password_md5){
	...
	if( $user_key && $user_passes[$user_key] == $password_md5)
		return true;
	else
		return false;	
}

function is_admin_md5( $username_md5, $password_md5){
	extract($GLOBALS);
	$user_names_md5 = array();
	for($i = 1; $i < count($user_names); $i++)
		$user_names_md5[$i] = md5($user_names[$i]);
...
/**
 * rrmdir
 * Removes the directory $dir even if it is not empty
 * @author	Jan Beneke -- lightgraffiti.de
 *
...
/**
 * mvdir
 * Move directory even if it is not empty.
 * @author	Jan Beneke -- lightgraffiti.de
 *
 * @param    $source 		string    source directory
 * @param	 $dest			string	  destination directory
 * @param	 $overwrite		boolean	  overwrite existing directories?
 * @access 	 public
 * @license  GPL v3
 */
function mvdir($source, $dest, $overwrite = true, $funcloc = NULL){
...
<?php
if(isset($_GET['dl'])){
	if( $fileurl == "" || $_GET['dl'] == "index.php" || $_GET['dl'] == "fi.php" || $_GET['dl'] == "fi.css"){
	?>
...
?>
		  <div id="download">
          <p align="center">Der Download von <a align="center" href="<?php echo $fileurl; ?>" style="color: #777; font-family:monospace;"><?php echo substr(strrchr($fileurl,'/'),1); ?></a> beginnt in <span id="download_time"></span> Sekunden.</p>
...
<?php	
} else if( $autoLogin || ( isset($_POST['u']) && isset($_POST['p']) && is_user($_POST['u'], $_POST['p']) ) || ( isset($_SESSION['u']) && isset($_SESSION['p']) && is_user_md5($_SESSION['u'],$_SESSION['p']) ) ) {	
	...
	// Set user permission
	if(isset($_POST['u']))
		$isAdmin = is_admin($_POST['u'], $_POST['p']);
	else if(isset($_SESSION['u']))
		$isAdmin = is_admin_md5($_SESSION['u'],$_SESSION['p']);
	...
	// move dir
		...
		if( file_exists($_POST['move_dirname']) && is_dir($_POST['move_dirname'] .'/' ) && $_POST['move_dirname'] != $_POST['move_dir_to']){
			...
			mvdir( $_POST['move_dirname'], $new_dirname);
		}else
			$err =true;
...
?>
    <div id="footnote">
    	<hr>
            <a href="javascript:toggle('copyright')">Copyright 2011 - <?php echo $year ?> by Jan Beneke (click for more)</a>
